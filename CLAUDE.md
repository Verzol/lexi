...
**M7 (post-Phase-1 launch hardening + accounts) is complete.** Per-IP rate limiting on login/register/resend (`app/ratelimit.py` — in-memory sliding window, deliberately no Redis/slowapi), IANA timezone validation at register (`schemas/auth.py`), a production secret guard at boot (`main.py._guard_production_secrets` — refuses to start with the dev `JWT_SECRET` when `COOKIE_SECURE=true`), **token revocation** via `users.token_version` (a `tv` claim on every token; logout bumps it, invalidating all outstanding access *and* refresh tokens), **email verification** (`POST /auth/verify-email` + `/auth/resend-verification`, signed short-lived "verify" JWT, soft gate — signup still auto-logs-in and the frontend just nags via `VerifyEmailBanner`), **Google sign-in** (`POST /auth/google`, Google Identity Services *ID-token* flow), a fix for the dashboard's commit-inside-a-GET-loop, and CI (`.github/workflows/ci.yml`: ruff + pytest + tsc + lint). Migrations `c1a2b3d4e5f6` (token_version) and `d2b3c4e5f6a7` (email_verified / auth_provider / google_sub, nullable password_hash). Both features are now **configured and verified end-to-end** (SMTP via a Gmail app password; a real Google OAuth Client ID in both tiers) — see `docs/SETUP_TODO.md`, where only the production checklist (§3) is outstanding. Supabase is migrated to `d2b3c4e5f6a7`.

**M8 is in progress — slice A only.** The quiz no longer trusts the client's `kind` (see Security model). Still to do, in this order:

- **B — audio + new question formats.** `cards.audio_url` already exists and is unused, so no migration. Plan was browser Web Speech API first (zero infra, matches the project's ethos), with `audio_url` as the escape hatch if real TTS files are ever wanted. Then *listening* (hear the term, type it) and *cloze* (`_blank_out_term()` in `quiz/service.py` already exists — promote it from an answer-hiding helper to a question format). **Any new format must be issued through `issue_question_token`**, or it reopens the hole slice A closed.
- **C — tech debt, done opportunistically, not as its own milestone.** `StudentApp.tsx` (~1080 lines) and `AdminApp.tsx` (~894) get split only where B has to touch them.

Note for scope discipline: SoW §8 lists "listening/speaking practice" as *Phase 2 backlog*. M8-B is deliberately pulling from that backlog now that Phase 1 is closed — it is not leftover Phase 1 work. Speaking/pronunciation scoring was explicitly cut (external API, real cost, poor accuracy).
...
- **Reminder emails go only to verified addresses** (`streaks/reminders.py:students_due_reminder`). Public signup means an address can be a typo or invented, and nagging one daily bounces forever and burns the configured sender's reputation. This is *not* a tightening of the soft gate — an unverified student still studies and still keeps a streak. Google accounts are born verified, so they're unaffected.
